...
- 仪表盘：
  ...
  - 当前文件夹移动控件
  - 新建/上传相关弹窗
  - 移动端优化：头部操作区改为可滚动按钮条，主要操作按钮与触控区尺寸增大
- 文件详情页：
  - 顶栏返回与操作按钮
  - 主区阅读或编辑
  - 移动端优化：操作按钮改为分层布局，信息区支持换行展示
- 账户页：用户修改当前账号密码
- 用户管理页：管理员新增/修改/删除用户
  - 移动端优化：提供卡片式用户管理视图，替代小屏表格阅读与操作问题
...
- 响应式策略：移动端优先优化核心业务页面（仪表盘、文件详情、用户管理）
...
- 已对核心页面做移动端体验优化，提升小屏幕下的触控效率与信息可读性
