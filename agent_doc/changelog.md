...
优化内容：
- 桌面与移动端头部操作区拆分显示
- 移动端新增横向可滚动操作按钮条（修改密码、用户管理、退出）
- 主操作按钮区改为更适合手机点击的网格布局
- 批量操作区、文件夹移动区输入控件与按钮高度提升
- 文件卡片底部“查看/编辑/删除”按钮在移动端加大触控面积

目标效果：
- 减少小屏拥挤与误触
- 降低关键操作（批量移动/删除、新建/上传）的点击成本
...
- 新增手机端卡片布局（`md` 以下）
  - 每个用户独立卡片展示
  - 在卡片内完成角色修改、密码重置、删除用户
- “返回仪表盘”按钮在小屏改为全宽样式
- “添加用户”表单控件统一增大触控区域

目标效果：
- 避免移动端横向滚动表格带来的阅读和操作困难
- 提升管理员在手机上的用户维护效率

### 11.4 本轮优化总结
- 这轮未引入新的业务逻辑，仅优化交互与布局
- 核心目标是提升手机与小屏设备下的可用性与操作稳定性
- 覆盖了当前最核心的三个业务界面：仪表盘、文件详情、用户管理